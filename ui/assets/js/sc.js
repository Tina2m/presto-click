...
const FLOW_LABELS = {
  ...
  sc_remove_multi_heavy: 'Remove cells with multiple IGH chains',
...
// ----- Dynamic labels for multi-heavy unit -----

const MULTI_HEAVY_VARIANTS = {

  IGH: { flowTitle: 'Remove cells with multiple IGH chains chains', short: 'IGH' },

  TRA: { flowTitle: 'Remove cells with multiple TRA chains', short: 'TRA' },

  TRB: { flowTitle: 'Remove cells with multiple TRB chains', short: 'TRB' },

  'TRA,TRB': { flowTitle: 'Remove cells with multiple TRA/TRB chains', short: 'TRA/TRB' },

};

function normalizeHeavyValue(v){
...
function describeMultiHeavy(value){

  const key = normalizeHeavyValue(value);

  return MULTI_HEAVY_VARIANTS[key] || MULTI_HEAVY_VARIANTS.IGH;

}

function dynamicUnitLabels(unitId, baseLabel, params){

  if(unitId !== 'sc_remove_multi_heavy'){

    return { label: baseLabel, flowTitle: flowLabel(unitId, baseLabel) };

  }

  const desc = describeMultiHeavy(params?.heavy_value);

  return { label: desc.flowTitle, flowTitle: desc.flowTitle, short: desc.short };
...
function buildUnitCard(u){
  ...
  if(u.id === 'sc_remove_multi_heavy'){
    ...
    decorateMultiHeavyCard(card, displayLabel);
...
function decorateMultiHeavyCard(card, baseLabel){
  const select = card.querySelector('select[name="heavy_value"]');
  const title = card.querySelector('.uc-title');
  if(!select || !title) return;

  const updateTitle = () => {
    const desc = describeMultiHeavy(select.value);
    title.textContent = `${baseLabel} (${desc.short})`;
  };

  const modeRow = document.createElement('div');
  modeRow.className = 'mt8';
  const modeName = `mh-mode-${Math.random().toString(36).slice(2)}`;
  modeRow.innerHTML = `
    <label class="muted">mode - toggle BCR/TCR</label>
    <div class="sc-mode-toggle">
      <label class="pill-input">
        <input type="radio" name="${modeName}" value="bcr" checked> BCR
      </label>
      <label class="pill-input">
        <input type="radio" name="${modeName}" value="tcr"> TCR
      </label>
    </div>
  `;
  select.insertAdjacentElement('beforebegin', modeRow);

  const radios = Array.from(modeRow.querySelectorAll(`input[name="${modeName}"]`));
  const ighOption = Array.from(select.options).find(opt => (opt.value || '').toUpperCase() === 'IGH');
  const applyMode = mode => {
    if(mode === 'bcr'){
      if(ighOption){
        ighOption.hidden = false;
        ighOption.disabled = false;
      }
      ...
      if(ighOption){
        ighOption.hidden = true;
        ighOption.disabled = true;
      }
      if((select.value || '').toUpperCase() === 'IGH'){
        select.value = 'TRA';
      }
    }
    updateTitle();
  };
  radios.forEach(radio => radio.addEventListener('change', () => applyMode(radio.value)));

  select.addEventListener('change', () => {
    const currentMode = radios.find(r => r.checked)?.value;
    if(currentMode === 'bcr' && (select.value || '').toUpperCase() !== 'IGH'){
      select.value = 'IGH';
    }
    updateTitle();
  });
...
function addToFlow(card, unitId, label){
  ...
  const labels = dynamicUnitLabels(unitId, label, params);

  FLOW.push({unitId, label: labels.label, flowTitle: labels.flowTitle, params});
...
function renderFlow(){
  ...
  if(FLOW.length === 0){
    ...
    FLOW.forEach((s,i)=>{
      ...
      const dynamic = dynamicUnitLabels(s.unitId, s.flowTitle || s.label || '', s.params);

      const labelText = dynamic.flowTitle || s.flowTitle || s.label;

      const titleText = dynamic.label || s.label;

      li.innerHTML = `<div class="flow-step">${i+1}</div>

                      <div class="flow-label" title="${esc(titleText || '')}">${esc(labelText || '')}</div>
...
function hydrateFlowFromPipeline(pipeline){
  ...
  const cloned = (pipeline || []).map(step => {
    ...
    const labels = dynamicUnitLabels(unitId, baseLabel, params);
